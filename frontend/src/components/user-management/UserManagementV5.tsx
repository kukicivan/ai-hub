...
import { MoreHorizontal, Pencil, Trash2, Key, CheckCircle, XCircle, Grid, List, Mail, Phone, MapPin, Calendar } from "lucide-react";
...
export default function UserManagementV5() {
    ...
    <Card className="group relative overflow-hidden transition-all hover:shadow-lg">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <Badge variant="outline" className="mt-1">{user.user_type?.name || "N/A"}</Badge>
            </div>
          </div>
          <DropdownMenu>
      ...
      <CardContent className="space-y-2 text-sm">
        <div className="flex items-center gap-2 text-muted-foreground">
          <Mail className="h-4 w-4" /><span className="truncate">{user.email}</span>
        </div>
        {user.phone && (
          <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {(user.city || user.country) && (
          <div className="flex items-center gap-2 text-muted-foreground">
            <MapPin className="h-4 w-4" /><span>{[user.city, user.country].filter(Boolean).join(", ")}</span>
          </div>
        )}
      </CardContent>
      <CardFooter className="flex items-center justify-between border-t pt-4">
        ...
        {user.email_verified_at ? (
          <Badge variant="secondary" className="text-green-600"><CheckCircle className="mr-1 h-3 w-3" />Verificiran</Badge>
        ) : (
          <Badge variant="secondary" className="text-yellow-600"><XCircle className="mr-1 h-3 w-3" />Pending</Badge>
        )}
      </CardFooter>
    </Card>
  );
  ...
  return (
    <UserManagementLayout version="V5" versionLabel="Card/Grid View" description="Moderni card-based prikaz sa grid/list toggle opcijom" onCreateUser={() => { setSelectedUser(null); setUserModalOpen(true); }} onExport={handleExport} onResetSettings={resetSettings} isExporting={isExporting}>
      {/* Toolbar */}
      <div className="flex items-center justify-between py-4 gap-4 flex-wrap">
        <div className="flex items-center gap-2 flex-1">
          <Input placeholder="Pretraži korisnike..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="max-w-sm" />
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={perPage.toString()} onValueChange={(v) => { setPerPage(parseInt(v)); setPage(1); }}>
            ...
            <SelectContent>{[6, 12, 24, 48].map((n) => <SelectItem key={n} value={n.toString()}>{n}</SelectItem>)}</SelectContent>
          </Select>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex h-[400px] items-center justify-center text-muted-foreground">Nema rezultata.</div>
      )}
